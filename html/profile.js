...
async function LoadProfileData() {
  try {
    const profileRes = await fetch('https://localhost:7204/api/Profile/GetProfile', {
      method: 'GET',
      headers: {
    ...
    document.getElementById('UserPhoto').src = profile.profilePictureLink;
    const postRes = await fetch('https://localhost:7204/api/Post/GetAllPosts', {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
...
function renderPosts(profile, posts) {
  ...
  posts.forEach(post => {
    let mediaHtml = '';
    if (post.postPhoto?.startsWith('https://localhost:7204') && post.postPhoto.length > 0) {
      ...
      <div class="card mb-3">
        <div class="card-body">
          <div class="d-flex align-items-start">
            <img class="rounded-circle me-3" src="${profile.profilePictureLink}" alt="${profile.name}" width="50" height="50">
            <div class="w-100">
...
function renderMedia(post) {
  const url = post.postPhoto?.replace("http://localhost", "https://localhost") || '';
  return url ? `
    <div class="mt-3">
      <img src="${url}" class="img-fluid rounded w-100" alt="media">
    </div>
  ` : '';
}
...
async function loadFriendsData() {
  try {
    const response = await fetch('https://localhost:7204/api/Profile/GetAllFriends', {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
        ...
        <div class="iq-friendlist-block">
          <div class="d-flex align-items-center justify-content-between">
            <div class="d-flex align-items-center">
              <a href="#"><img src="${friend.profilePictureLink}" alt="profile-img" class="img-fluid"></a>
